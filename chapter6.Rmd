...
- this weeks exercise have been pretty frustrating. Many times I have been wondering if I do not understand or is there just a mistake or some kind of shortage in the materials
- The Datacamp nor the book adequately help to interpret the results from the mixed effect models
- I really would have wanted to learn more about longitudinal methods
...
Next, I will first fit a linear model with the mean weight as the response and group as explanatory variable. The results of the model are shown below.
...
As can be seen from the summary table above, the positive regression coefficients of both groups are statistically significant (p < 0.001), meaning that the weight of the rats differs between groups. Next, I will add the baseline measurement (day 1) from the original data to the summary data and add it in the linear regression model. The results are presented below.
...
When taking the baseline into account results are quite different! According to the results from the linear regression model the coefficient of group 2 or 3 are not statistically significant (when considering p < 0.05 as the threshold). Analysis of variance provides similar results. 
...
Note that even though the significance of the results or coefficient is commented multiple times in the chapter 9, there are no p-values or explanations how to interpret the significance of the results, which I found very annoying. Here, I installed _lmerTest_-package which adds p-values to the lrm-summary tables.
...
The last thing to do is to draw one more figure which shows the observed and fitted BPRS profiles. Based on the above presented results I will the random intercept and slope model without interaction term for fitted values. The two figures are presented below. They illustrate how well the model fits the observed data. Seems to fit somewhat roughly and there are also quite big differences between the observed and fitted lines, but everyone can take a look and make a judgement of their own.
