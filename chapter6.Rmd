...
After removing the outlier, the differences between groups are even clearer as can be seen in the picture above. Here, I have to improvise as it is impossible to conduct same analyses that were implemented in the Datacamp exercise (T-test). I apply a Hotelling’s test to investigate the difference between groups. Hotelling's t-test can be used when there are more than two groups, which is the case here ([see chapter 18 of MABS](https://mooc.helsinki.fi/pluginfile.php/192850/course/section/7333/MABS4IODS-Part4.pdf). I found a Hotelling packege that include the test: see documentation on [Hotelling's package](https://cran.r-project.org/web/packages/Hotelling/Hotelling.pdf). I'm not sure if that is the right one, but it is my best guess. The test results show that all the groups' means are statistically significantly different from each other.
...
hstat <- hotelling.stat(mean ~ Group, data = rats_summary_filt)
