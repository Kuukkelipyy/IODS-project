...
The original RATS data included 16 observations of 13 variables, that is 16 rats with multiple measurements over time (and a few other variables). Here, the data is transformed from wide to long format by time (measurements), meaning that each time point (i.e. measurement) is one observation and thus every rat has as many observations as there are measurements. In the long format there are 176 observations of 4 variables, which are:

- ID: unique id of participants (rats)
- Group: the treatment group; rats were grouped into 3 groups with different diets (8 rats in group 1 and 4 in groups 2 and 3)
