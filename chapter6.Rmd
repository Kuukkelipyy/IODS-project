...
After removing the outlier, the differences between groups are even clearer as can be seen in the picture above. Nevertheless, let's apply a Hotelling’s T-test to investigate hte difference between the groups. Hostelling's t-test can be used when there are more than two groups, which is the case here ([see chapter 18 of MABS](https://mooc.helsinki.fi/pluginfile.php/192850/course/section/7333/MABS4IODS-Part4.pdf). HOW?

[hotelling's package](https://cran.r-project.org/web/packages/Hotelling/Hotelling.pdf)

, and also calculate a confidence interval for the differences difference. For t-test I will use the data without the one outlier. 

```{r message=FALSE, warning=FALSE}


# Perform a two-sample t-test
t.test(mean ~ Group, data = rats_summary_filt, var.equal = TRUE)
